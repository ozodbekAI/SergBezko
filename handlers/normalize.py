...
from keyboards import (get_back_button_normalize, get_generation_menu, get_normalize_menu, get_model_types, 
...
async def normalize_own_model(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text, reply_markup=get_back_button("norm_own_model"))
...
async def normalize_photo_received(message: Message, state: FSMContext):
    if message.media_group_id:
        ...
        await message.answer("❌ Пожалуйста, отправляйте фото по ОДНОМУ (не альбомом).", 
                           reply_markup=get_back_button(back_step))
        return
    ...
    try:
        ...
        await message.answer(str(e), reply_markup=get_back_button("gen_normalize"))
        return
        ...
        await message.answer("❌ Ошибка при обработке фото. Попробуйте другое фото.", 
                           reply_markup=get_back_button("gen_normalize"))
        return
    ...
    if mode == "own_model":
        if photo_count < 2:
            ...
            await message.answer(f"✅ Фото {photo_count}/2 получено.\n\nОтправьте второе фото.", 
                               reply_markup=get_back_button("waiting_photo_1"))
        else:
            ...
            async with async_session_maker() as session:
                ...
                if not has_balance:
                    await message.answer("❌ Недостаточно кредитов на балансе.\n\nПополните баланс в разделе 'Мой кабинет.'", 
                                       reply_markup=get_back_button("norm_own_model"))
                    await state.clear()
                    return
            
            await message.answer(f"Будет списано {cost} кредита.\n\nПродолжить?", 
                               reply_markup=get_confirmation_keyboard(cost, "confirming_own"))
...
async def normalize_invalid_input(message: Message, state: FSMContext):
    ...
    await message.answer(text + "\n\nПожалуйста, отправьте фото (как фото или как файл).", reply_markup=get_back_button(back_step))
...
async def select_model_type(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(model_type=model_id, cost=cost)
    ...
    async with async_session_maker() as session:
        ...
        if not has_balance:
            await callback.message.edit_text("❌ Недостаточно кредитов на балансе.\n\nПополните баланс в разделе 'Мой кабинет.'", 
                                           reply_markup=get_back_button("selecting_model_type"))
            await state.clear()
            return
    ...
    try:
        ...
        await callback.message.edit_text("❌ Тип модели не найден. Вернуться к выбору?", 
                                       reply_markup=get_model_types(config_loader.model_types.get("model_types", [])))
        await state.set_state(NormalizeStates.selecting_model_type)
        return
    
    await callback.message.edit_text(f"Выбрана модель: {model['name']}\n\nБудет списано {cost} кредита.\n\nПродолжить?", 
                                    reply_markup=get_confirmation_keyboard(cost, "confirming_new"))
    await state.set_state(NormalizeStates.confirming)
...
async def back_navigation_normalize(callback: CallbackQuery, state: FSMContext):
    ...
    if back_data in ["norm_own_model", "norm_new_model"]:
        ...
        await callback.message.edit_text(
            ...
            reply_markup=get_normalize_menu()
        )
        return
    ...
    if back_data == "selecting_model_type":
        await state.set_state(NormalizeStates.waiting_for_photos)
        await state.update_data(mode="new_model", photo_urls=[])
        await callback.message.edit_text(
    ...
    if back_data == "confirming_new":
        ...
        models = config_loader.model_types.get("model_types", [])
        await callback.message.edit_text(
            "Выберите тип фотомодели:", 
            reply_markup=get_model_types(models)
        )
        return
...
def get_back_button(current_step: str):
    ...
    builder.row(InlineKeyboardButton(text="◀️ Назад", callback_data=f"norm_back_{current_step}"))
